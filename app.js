...
function normalizeBook(raw = {}) {
  ...
  const detailPath = String(pick(raw, ['detailPath', 'detail_path', 'path', 'detailUrl', 'detail_url'], ''));
  return {
...
function normalizeList(payload) {
  return arrayFrom(unwrap(payload))
    .map(normalizeBook)
    .filter(book => book.id && !unsafe.test([book.title, book.author, book.summary, book.genre, ...book.tags].join(' ')));
}
...
async function api(path) {
  const response = await fetch(/^https?:\/\//i.test(path) ? path : API + path, {
...
function bookKey(book) {
  return String(book?.id || book?.detailPath || book?.title || '');
}
...
function toggleShelf(book) {
  if (isSaved(book)) state.shelf = state.shelf.filter(item => bookKey(item) !== bookKey(book));
  else state.shelf = [book, ...state.shelf];
  localStorage.setItem(SHELF_KEY, JSON.stringify(state.shelf));
  toast(isSaved(book) ? 'Added to My Shelf' : 'Removed from My Shelf');
}
...
function sourceQuery(book) {
  return book.detailPath ? `?source=${encodeURIComponent(book.detailPath)}` : '';
}

function novelHref(book) {
  return `/novel/${encodeURIComponent(book.id)}${sourceQuery(book)}`;
}

function chapterHref(chapter, book, index = 0) {
  return `/chapter/${encodeURIComponent(chapter.id)}/${encodeURIComponent(book.id)}/${index}${sourceQuery(book)}`;
}

function card(book, rank = '') {
  rememberBooks([book]);
  return `<article class="card">
    <a class="poster-link" href="${novelHref(book)}">${poster(book, rank)}</a>
    <h3 title="${esc(book.title)}"><a href="${novelHref(book)}">${esc(book.title)}</a></h3>
    <small>${esc(book.author)}</small>
    <div class="card-actions">
      <a class="more-btn" href="${novelHref(book)}">MORE</a>
      <button type="button" class="shelf-btn ${isSaved(book) ? 'saved' : ''}" data-save="${esc(bookKey(book))}">${isSaved(book) ? '✓ On shelf' : '+ My Shelf'}</button>
    </div>
  </article>`;
}
...
function hero(book, total) {
  if (!book) {
    return `<section class="hero empty-hero"><div class="hero-content"><div class="hero-copy"><div class="eyebrow">NOVELHUB · FOXY EDITION</div><h1>DISCOVER YOUR NEXT <i>STORY.</i></h1><p>The catalogue is loading. Search for a title, author or genre.</p><form id="heroSearch" class="hero-search"><input id="heroInput" type="search" placeholder="Search novels, authors, genres..."><button type="submit">SEARCH</button></form></div></div></section>`;
  }
...
function renderHome() {
  ...
  $('#heroSearch')?.addEventListener('submit', event => {
    ...
    navigate('/search/' + encodeURIComponent($('#heroInput')?.value.trim() || ''));
  });
...
function getReaderPrefs() {
  ...
  try {
    ...
    return {
      ...defaults,
      ...saved,
      fontSize: Math.max(16, Math.min(28, Number(saved.fontSize) || defaults.fontSize)),
      lineHeight: Math.max(1.65, Math.min(2.2, Number(saved.lineHeight) || defaults.lineHeight)),
      width: ['narrow', 'comfortable', 'wide'].includes(saved.width) ? saved.width : defaults.width,
      theme: ['dark', 'sepia', 'black'].includes(saved.theme) ? saved.theme : defaults.theme,
    };
  } catch (_) { return defaults; }
}

function saveReaderPrefs(value) {
  localStorage.setItem(READER_PREFS_KEY, JSON.stringify(value));
}
...
async function getBook(id, source = '') {
  ...
  for (const candidate of candidates) {
    try {
      const raw = unwrap(await api(candidate));
      const book = normalizeBook(raw);
  ...
  // Last-resort resolver: use the public search endpoint to recover the API detail path.
  try {
    ...
    if (found) {
      rememberBooks([found]);
      if (found.detailPath) {
        const raw = unwrap(await api(normalizeDetailPath(found.detailPath, id)));
        const book = normalizeBook(raw);
        book.id = book.id || String(id);
...
function inlineSummaryMore() {
  document.querySelectorAll('.detail .summary').forEach(summary => {
    ...
    button.addEventListener('click', event => {
      ...
      button.dataset.open = String(!expanded);
      button.textContent = expanded ? 'MORE' : 'LESS';
      text.textContent = expanded ? shortText + '… ' : full + ' ';
    });
    summary.textContent = '';
...
async function renderNovelRoute(parts, params, token) {
  ...
  const book = await getBook(id, source);
  if (token !== state.routeToken) return;
...
function renderReader(book, chapters, current, chapter, source) {
  ...
  app.innerHTML = `<div class="reader-shell" id="readerRoot" data-width="${prefs.width}" data-theme="${prefs.theme}">
    <div class="reader-progress"><div id="readerProgressFill"></div></div>
    <header class="reader-topbar"><div class="reader-top-left"><a class="reader-back" href="${novelHref(book)}">← <span>BACK</span></a><div class="reader-book"><small>${esc(book.title)}</small><b>${esc(chapter.title === 'Chapter' ? current.title : chapter.title)}</b></div></div><div class="reader-top-tools"><span id="readerProgressText" class="reader-progress-text">0%</span><button id="readerOpenSettings" class="reader-tool" type="button">Aa</button><button id="readerOpenChapters" class="reader-tool" type="button">☰</button></div></header>
    ${readerSettings(prefs)}
    <main class="reader-main"><div class="reader-kicker"><span>${esc(book.title)}</span><i>·</i><span>CHAPTER ${currentIndex + 1}</span></div><article class="reader-article" id="readerArticle" tabindex="-1"><h1>${esc(chapter.title === 'Chapter' ? current.title : chapter.title)}</h1>${paragraphs.map(p => `<p>${esc(p)}</p>`).join('')}</article><nav class="chapter-nav" aria-label="Chapter navigation">${previous ? `<a class="chapter-nav-btn" href="${chapterHref(previous, book, currentIndex - 1)}"><span>←</span><small>PREVIOUS</small><b>${esc(previous.title)}</b></a>` : '<span class="chapter-nav-btn disabled"><span>—</span><small>PREVIOUS</small><b>Beginning</b></span>'}<a class="chapter-nav-center" href="${novelHref(book)}"><span>CHAPTER</span><b>${currentIndex + 1} / ${chapters.length || '—'}</b><small>VIEW ALL</small></a>${next ? `<a class="chapter-nav-btn next" href="${chapterHref(next, book, currentIndex + 1)}"><small>NEXT</small><b>${esc(next.title)}</b><span>→</span></a>` : '<span class="chapter-nav-btn disabled next"><small>NEXT</small><b>End of story</b><span>—</span></span>'}</nav></main>
    <button id="readerTop" class="reader-top" type="button">↑</button>
    <div id="readerOverlay" class="reader-overlay" hidden></div><aside id="readerDrawer" class="reader-drawer" aria-hidden="true"><div class="reader-drawer-head"><div><div class="eyebrow">NOVEL</div><h2>${esc(book.title)}</h2></div><button id="readerCloseChapters" type="button">×</button></div><input id="readerChapterSearch" class="reader-chapter-search" type="search" placeholder="Find a chapter..."><div id="readerChapterList" class="reader-chapter-list">${chapters.map((item, index) => `<a class="reader-chapter ${index === currentIndex ? 'active' : ''}" href="${chapterHref(item, book, index)}"><span>${String(index + 1).padStart(2, '0')}</span><b>${esc(item.title)}</b></a>`).join('')}</div></aside>
  </div>`;
  ...
  $('#readerChapterSearch')?.addEventListener('input', event => {
    ...
    document.querySelectorAll('.reader-chapter').forEach(link => { link.hidden = Boolean(q) && !link.textContent.toLowerCase().includes(q); });
  });
  ...
  const onScroll = () => {
    ...
    requestAnimationFrame(() => {
      ...
      $('#readerProgressFill')?.style.setProperty('transform', `scaleX(${value})`);
...
async function renderChapterRoute(parts, params, token) {
  ...
  const book = await getBook(bookId, source);
  if (token !== state.routeToken) return;
  const chapters = await getChapters(book.id);
  if (token !== state.routeToken) return;
  ...
  if (token !== state.routeToken) return;
  renderReader(book, chapters, current, chapter, source);
}
...
// Capture internal links so navigation stays a real pathname route instead of a hash route.
...
// One-time migration for older Novel Hub hash URLs already saved/bookmarked.
